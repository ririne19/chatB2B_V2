...
import type { TypingUser } from "@/src/components/TypingIndicator";
...
export default function ConversationPage() {
  ...
  const title =
    conversation?.title ?? fetchedConversation?.title ?? "Conversation";
  ...
  useEffect(() => {
    if (!socket || !id) return;
    const onTyping = (payload: { userId: string; firstName: string; lastName: string; active?: boolean }) => {
      const { userId, firstName, lastName, active } = payload;
      ...
      setTypingUsers((prev) => {
        ...
        next.push({ userId, firstName, lastName });
        return next;
      });
      const t = setTimeout(() => {
        delete typingTimeoutsRef.current[userId];
